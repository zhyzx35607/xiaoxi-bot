...
建议 Python 3.10+。直接使用 `requirements.txt` 安装已验证版本的 `websockets` 和 `aiohttp`。

B站推送只有在 NapCat 明确返回成功，或通过最近群消息历史确认消息已经出现后，才会更新动态 ID 和时间水位。超时但未确认的消息会留到下一轮处理，避免漏推；下一轮发送前也会先查历史，避免超时后的重复推送。
