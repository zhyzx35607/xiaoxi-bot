...
app/
 ??? config.py             ?????????????????
 ??? logging_setup.py      bot.log / chat.log ???
 ??? bootstrap.py          Client?Dispatcher ?????????
bot/
 ??? ai/                   AI ???Provider????????????????????
 ??? commands/             ???? admin/moderation/queries/media/fun/system ????
 ??? events/               ???????????/????????
 ??? transport/            OneBot WebSocket???????
 ??? integrations/         Bilibili?TouchGal?UAPI?NapCat ? canonical ??
 ??? services/             ??????????????????
 ??? security/             URL ?????????
 ??? storage/              ?? JSON ??
 ??? dispatcher.py         ???????????
