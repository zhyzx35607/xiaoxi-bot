...
群主或最高主人在群里用 `/agent 管群 on` 可开启本群 Agent 管群自治（默认关闭，`/agent 管群 off` 关闭）。开启后小汐是群主/管理时会定时巡检本群消息：撤回、限时禁言、处理入群申请等低风险处置在门控（按群开关、bot 实时管理角色、目标保护、每日配额）全部通过时自动执行，并记入行动时间线；踢人等高风险处置仍需 `/确认 CODE` 人工确认后才会执行。每群每日处置配额默认 20 次、禁言时长上限默认 600 秒，可在 config.json 的 `agent` 段用 `moderation_daily_limit` 和 `moderation_ban_max_seconds` 调整。
