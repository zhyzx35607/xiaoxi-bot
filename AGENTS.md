...
- AI 工具只暴露完成任务所需的最小能力。管理 API 只能经 Agent 网关的受控管理层（`MODERATION_ACTIONS`）暴露，按群开关、bot 实时角色复核、目标保护和每日配额逐次门控，不得直接交给模型自由调用。
- 高风险 Agent 计划（含踢人、入群申请处理等高风险群管动作）必须经过 verifier、冻结工具集合和确认流程后执行；低风险群管动作（撤回、限时禁言）仅在上述门控全部通过时才可自动执行。
